...
shinyApp(ui = fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput(inputId = "gr7", "Mu:",
                   min = 0.005, max = 10000,
                   value = 2, step = 1),
      numericInput(inputId = "gr10", "Sigma:",
                   min = 0, max = 10000,
                   value = 4, step = 1),
      numericInput(inputId = "gr3", "Number of S:",
                   min = 0, max = 100000,
                   value = 595, step = 1),
      numericInput(inputId = "gr11", "Number of E:",
                   min = 0, max = 100000,
                   value = 20, step = 1),
  ...
  params <- reactive(c(beta = input$gr1, gamma = input$gr2, mu = input$gr7, sigma = input$gr10))
  x <- reactive(c(time = 0, S = input$gr3, E=input$gr11, I = input$gr4, R = input$gr5))
  #maxstep1 < - reactive((maxstep = input$gr8))
  #nsims1 < - reactive(nsims = input$gr9)
  #times <- reactive({seq(input$gr6[1],input$gr6[2])})
  # Solve and plot.

  val <- reactive(seir_nsimul(x = x(), params = params(), maxstep = input$gr8, nsims = input$gr9))



  output$plot <- renderPlot({

    par(mfrow=c(4,1),  mar=c(2,4,1,1)) #mar=c(4,4,1,2)

    plot(S~time,data=val(),type='n')
    d_ply(val(),".n",function(x)lines(S~time,data=x,col=.n))

    plot(E~time,data=val(),type='n')
    d_ply(val(),".n",function(x)lines(E~time,data=x,col=.n))
