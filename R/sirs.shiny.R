...
shinyApp(ui = fluidPage(
  ...
  parameters <- reactive(c(beta = input$gr1, gamma = input$gr2, mu = input$gr7, xi = input$gr8))
  initials <- reactive(c(S = input$gr3, I = input$gr4, R = input$gr5))
  times <- reactive({seq(input$gr6[1],input$gr6[2])})
  # Solve and plot.

  val <- reactive(sirs_birthsdeaths(pars = parameters(), init = initials(), time = times())$results)
